...
1. **Compose each brief from the sub-workflow's persona in AgentProfiles.yaml** (workflow_mappings) to get proper trait/voice mapping
2. **Send a single message with multiple agent/Task tool calls** for parallel execution

```
<example>
User approves: "pursue 1,2,3"

Step 1: Compose briefs from personas (no external tooling required)
┌─────────────────────────────────────────────────────────────────────────┐
│ Brief 1: persona=collector (DomainRecon)                               │
│   traits: intelligence, technical, systematic                          │
│   task: "Domain recon for X"                                           │
│                                                                         │
│ Brief 2: persona=collector (EmailRecon)                                │
│   traits: intelligence, analytical, systematic                         │
│   task: "PGP analysis for Y"                                           │
│                                                                         │
│ Brief 3: persona=analyst (CompanyProfile)                              │
│   traits: intelligence, business, synthesizing                         │
│   task: "Company profile for Z"                                        │
└─────────────────────────────────────────────────────────────────────────┘

Step 2: Dispatch agents with the composed briefs (single message, parallel)
┌─────────────────────────────────────────────────────────────────────────┐
│ Agent/Task 1: prompt=[persona brief for domain recon]                  │
│ Agent/Task 2: prompt=[persona brief for PGP analysis]                  │
│ Agent/Task 3: prompt=[persona brief for company profile]               │
│                                                                         │
...
4. Store all entities and relationships via the memory adapter
   (SKILL.md § Memory Adapter):
   - Group: "osint-investigation-{slug}-{year}" (e.g. investigation id
     OSINT-INV-2026-001 → group osint-investigation-INV-2026-001)
...
💾 These leads are stored via the memory adapter in group osint-investigation-{slug}-{year},
   tagged "deferred" (Path 2: under a "Deferred leads" heading in the group findings file)
...
Generated by OSINT Investigation Orchestrator
💾 Stored via memory adapter: osint-investigation-{slug}-{year} (MuninnDB group | local findings file)
...
All investigation data is stored via the memory adapter (SKILL.md § Memory Adapter), group "osint-investigation-{slug}-{year}":

### Primary Investigation Group
```
Group: osint-investigation-{slug}-{year}
(e.g. investigation id OSINT-INV-2026-001 → osint-investigation-INV-2026-001)
tags: ["osint-investigation-{slug}-{year}", "osint"]

Findings stored (one memory/entry each):
...
3. All relationships
   - Entity-to-entity links (muninn_link relates_to / supports)
...
Each deferred pivot is stored in the same investigation group with an extra "deferred" tag (Path 1) or under a "Deferred leads" heading in the group findings file (Path 2):

```
Findings stored (for each deferred pivot):
...
To find all deferred leads from an investigation (Path 1):
```
muninn_recall(
  context: ["deferred pivot investigation OSINT-INV-2026-001"],
  tags_any: ["osint-investigation-INV-2026-001"]
)
```
...
```
muninn_recall(
  context: ["deferred pivot OSINT"],
  tags_any: ["deferred"]
)
```
(Path 2 equivalent: grep the "Deferred leads" headings in ./osint-findings/.)

### Resume Investigation Trigger

When user says "resume investigation" or "pursue deferred leads":
1. Recall deferred leads via the memory adapter (muninn_recall / read findings files)
